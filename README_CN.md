...
从 npm 包本身还原 **Claude Code**（`@anthropic-ai/claude-code`）的完整 TypeScript 源码——无需 GitHub 镜像，无需任何泄漏渠道，结果完全可验证。
...
```
recovered/
├── src/               ← Anthropic 自己的 TypeScript 源码（1906 个文件）
│   ├── utils/
│   ├── services/
│   ├── bootstrap/
│   ├── tools/
...
│   └── ...
└── node_modules/      ← 被一同打包的依赖（同样带有源码）
```

使用 `--only-anthropic` 参数可跳过 `node_modules`，只还原 Anthropic 自己的 1906 个文件。
...
本仓库**不包含任何 Anthropic 的源代码**。
仓库只包含一个脚本，用于读取你自己从 npm 官方 registry 下载的文件。
该 npm tarball 由 Anthropic 公开发布，任何人均可下载。
从 source map 中读取 `sourcesContent` 是数百万开发者每天都在使用的标准浏览器 / Node.js 工具链操作。
